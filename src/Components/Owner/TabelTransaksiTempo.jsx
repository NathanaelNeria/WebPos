...
const TabelTransaksiTempo = ({ page, invoiceData, loading }) => {
  ...
  const handleStatusChange = async (id, value) => {
    try {
      const ref = doc(db, "transaksiPenjualan", id);
      ...
      setData((prev) =>
        prev.map((item) =>
          item.id === id ? { ...item, status_owner: value } : item,
        ),
      );
    } catch (err) {
              ...
              >
                ...
                <td
                  className={`rounded-md px-2 py-1 text-sm font-semibold ${
                    row.status_owner === "LUNAS"
                      ? "bg-green-100 text-green-700"
                      : row.status_owner === "ACC"
                        ? "bg-blue-100 text-blue-700"
                        : row.status_owner === "MAP"
                ...
                >
                  <select
                    value={row.status_owner || "PENDING"}
...
export default TabelTransaksiTempo;
